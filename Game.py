...
class Game:
    def __init__(self):
        ...
        if LANDSCAPE == 1:
            ...
            self.background = NightBackground()
        else:
            quit()
    ...
    def restart_or_quit(self):
        ...
        if key[pygame.K_RETURN]:
            self.obstacle = Columns()
            self.bird = FlappyBird() if SELECT_BIRD == 1 else RedFlappyBird()
            self.is_game_over = False
            self.score = 0
    ...
    def high_scorer(self, current_score):
        ...
        """
        if self.high_score < current_score // 2:
            self.high_score = current_score // 2
